...
// Selección por defecto: última carrera con resultados, o la más reciente pasada
if (isset($_GET['carrera'])) {
    $idCarreraVer = (int)$_GET['carrera'];
} else {
    // Buscar la última carrera con resultados
    $defaultId = 0;
    foreach (array_reverse($carreras) as $c) {
        if ((int)$c['tiene_resultados'] > 0) { $defaultId = (int)$c['id_carrera']; break; }
    }
    // Si no hay resultados, seleccionar la última carrera pasada
    if (!$defaultId) {
        foreach (array_reverse($carreras) as $c) {
            if ($c['fecha'] <= date('Y-m-d')) { $defaultId = (int)$c['id_carrera']; break; }
        }
    }
    // Si todo falla, primera carrera
